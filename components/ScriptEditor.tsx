...
import { ScriptSegment, TextAlign } from '../types';
            ...
            <div className="flex-1 flex flex-col min-h-0">
                {hook.activeTab === 'write' && (
                    <div className="flex-1 flex flex-col p-4 gap-3 min-h-0">
                        <div className="p-2 bg-zinc-950 rounded-lg border border-zinc-800 flex flex-col lg:flex-row gap-2 justify-between">
                            <div className="flex items-center gap-2 flex-1 border-b lg:border-b-0 lg:border-r border-zinc-800 pb-2 lg:pb-0 lg:pr-2">
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex bg-zinc-900 p-1 rounded-lg border border-zinc-800">
                                    {(['left', 'center', 'right'] as TextAlign[]).map((align) => (
                                        <button 
                                            key={align} 
                                            onClick={() => hook.updateAlignment(align)} 
                                            className={`p-1.5 rounded transition-colors ${hook.textAlign === align ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                                            title={`Align ${align}`}
                                        >
                                ...
                                <Button variant="ghost" size="sm" onClick={() => { hook.syncSegments(); hook.showToast("Saved to history"); }}>Sync</Button>
                            </div>
                        </div>
                        <textarea 
                            ref={textareaRef} 
                            dir="auto"
                            className="flex-1 bg-zinc-950 text-zinc-100 p-4 rounded-lg resize-none border border-zinc-800 font-mono text-base outline-none focus:ring-1 focus:ring-indigo-500 transition-all" 
                            style={{ textAlign: hook.textAlign }}
                            value={hook.rawText} 
                            onChange={e => hook.setRawText(e.target.value)} 
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar select-text">
                        <div className="shrink-0 p-2 flex justify-center gap-2 mb-4">
                            {['text-yellow-400', 'text-red-500', 'text-green-400', 'text-cyan-400', 'text-purple-400', ''].map((c, i) => (
                                <button key={i} onMouseDown={e => { e.preventDefault(); applyColor(c); }} className={`w-6 h-6 rounded-full border border-zinc-700 ${c || 'bg-zinc-800'}`} style={{ color: c ? undefined : 'transparent', backgroundColor: c ? 'currentColor' : undefined }} />
                            ))}
                        </div>
                        {segments.map(seg => (
                            <div key={seg.id} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800" style={{ textAlign: seg.textAlign || 'center' }}>
                                <div className="flex flex-wrap gap-1.5 mb-3 justify-center" style={{ justifyContent: (seg.textAlign === 'left' ? 'flex-start' : seg.textAlign === 'right' ? 'flex-end' : 'center') }}>
                                    {seg.words.map((w, i) => (
                                        <span key={i} dir="auto" data-seg-id={seg.id} data-word-idx={i} className={`text-lg font-medium ${w.color || 'text-zinc-200'}`}>{w.text}</span>
                                    ))}
                                </div>
                                <div className="flex items-center gap-3">
                                    <input type="range" min="100" max="15000" step="100" value={seg.duration} onChange={e => hook.onSegmentsChange(segments.map(s => s.id === seg.id ? { ...s, duration: parseInt(e.target.value) } : s))} className="flex-1 accent-indigo-500 h-1.5 bg-zinc-800 rounded-lg appearance-none" />
                ...
                {hook.activeTab === 'history' && (
                    <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
                        {hook.savedScripts.map(s => (
                            <div key={s.id} onClick={() => hook.selectFromHistory(s)} className={`p-3 rounded-lg border cursor-pointer ${hook.currentScriptId === s.id ? 'border-indigo-500 bg-indigo-500/10' : 'border-zinc-800 bg-zinc-950 hover:border-zinc-600'}`}>
                                <div className="font-medium text-white">{s.title}</div>
                                <div className="text-xs text-zinc-500">{new Date(s.date).toLocaleDateString()} • {s.segments.length} segments • {s.textAlign || 'center'}</div>
                            </div>
                        ))}
                    </div>
                )}
