...
export const TeleprompterView: React.FC<TeleprompterViewProps> = ({ segments, onClose }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex flex-col overflow-hidden ${isCameraActive ? 'bg-black/30' : 'bg-black'} text-white`}>
                ...
                >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-2 border-t border-zinc-800">
                            <label className="text-zinc-500 block mb-1">Camera Zoom: {config.videoScale}x</label>
                        ...
                        <div>
                            <label className="text-zinc-500 block mb-1">Brightness</label>
                            <input type="range" min="0.5" max="2" step="0.1" value={config.brightness} onChange={e=>setConfig({...config, brightness: parseFloat(e.target.value)})} className="w-full accent-indigo-500 h-1.5 bg-zinc-800 rounded-lg appearance-none"/>
                        </div>
                    </div>
                </div>
            )}
            ...
            {config.showTimer && (
                <div className={`absolute top-8 left-8 z-30 font-mono text-5xl font-bold text-white/20 drop-shadow-lg select-none ${config.isMirrored ? 'scale-x-[-1]' : ''}`}>
            ...
            {/* Centered Pill Bottom Control Bar */}
